...
function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Box className={"flex w-300 border-b-2 p-4 relative"}>
      <NavLink to={`/produkty/${product.product_id}`} className={"h-full left-0 top-0 absolute w-full z-[1]"} />
      <img src={`${API_URL}/${product.image}`} alt="product-image" style={{ height: "200px" }} />
      <Box className={"grid w-full"}>
        <ProductInfo product={product} classNames={"ml-4"} />
        <Box className={"flex flex-col justify-between items-center gap-2 justify-self-end z-10"}>
