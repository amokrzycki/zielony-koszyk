    ...
    <Box className={"flex flex-row gap-4 p-4 justify-center"} id="main-wrapper">
      <Box
        ...
        <Box className={"p-4"}>
          ...
          <Typography>
            {orderInfo.shippingAddress.first_name} {orderInfo.shippingAddress.last_name}
          </Typography>
          {orderInfo.shippingAddress.customer_type === CustomerType.COMPANY && (
            <>
              <Typography>Firma: {orderInfo.shippingAddress.company_name}</Typography>
          ...
          <Typography>Adres: {generateOrderAddress(orderInfo.shippingAddress)}</Typography>
          ...
          {orderInfo.same_address ? (
            ...
            <>
              <Typography>
                {orderInfo.billingAddress.first_name} {orderInfo.billingAddress.last_name}
              </Typography>
              {orderInfo.order_type === OrderType.COMPANY && (
                <>
                  <Typography>Firma: {orderInfo.billingAddress.company_name}</Typography>
                  <Typography>NIP: {orderInfo.billingAddress.nip}</Typography>
                </>
              )}
              <Typography>Telefon: {orderInfo.billingAddress.phone}</Typography>
              <Typography>Adres: {generateOrderAddress(orderInfo.billingAddress)}</Typography>
            </>
          )}
